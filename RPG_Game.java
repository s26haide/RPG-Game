...
public class RPG_Game
{
    ...
    throws Exception
    {
        ...
        String fileName = "C:\\Users\\Sakin\\Desktop\\Development\\CS Projects\\SakinHaider_FP11\\FE-TWR.wav"; //Import music file
        InputStream in = new FileInputStream(fileName); //Create new Input Stream with fileName
        AudioStream audioStream = new AudioStream(in); //Create new audioStream to play audio
        AudioPlayer.player.start(audioStream); //Play audio using AudioPlayer
        ...
        while(a[b].getHealth() > 0 && MC.getHealth() > 0)//Battle while monster and player have more than 0 health
        {
            ...
            if (timesRun == 1) // First turn of the battle, displays information
            {
                ...
                {   //Plays Audio for when Monster appears
                    fileName = "C:\\Users\\Sakin\\Desktop\\Development\\CS Projects\\SakinHaider_FP11\\monster.wav";
                ...
                if(a[b].getHealth() > 0) //if monster is alive, it will attack
                {
                    ...
                    if(MC.getHealth() == 0)//if player dies
                    {
                        { //play Sound Effect for game over
                            fileName = "C:\\Users\\Sakin\\Desktop\\Development\\CS Projects\\SakinHaider_FP11\\Sound-Effect3.wav";
                            InputStream in1 = new FileInputStream(fileName);
                            AudioStream audioStream1 = new AudioStream(in1);
                            AudioPlayer.player.start(audioStream1);
                        }
            ...
            if (a[b].getHealth() == 0) //if monster died and battle is over
            {
                {   //play Sound Effect for end of battle
                    fileName = "C:\\Users\\Sakin\\Desktop\\Development\\CS Projects\\SakinHaider_FP11\\Sound-Effect24.wav";
                    InputStream in1 = new FileInputStream(fileName);
                    AudioStream audioStream1 = new AudioStream(in1);
                    AudioPlayer.player.start(audioStream1);
                }
                ...
                else if (input.equalsIgnoreCase("2")) //if town option is chosen
                {
                    ...
                    {
                        System.out.println("The next area is filled with a stronger monsters.");
                        delay(1);
